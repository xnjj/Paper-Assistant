...
# ---------------------------- 检索配置 ----------------------------
TOP_K = 5                              # 本地向量召回候选 chunk 数，默认50
KEYWORD_RECALL_K = 5                   # 关键词 BM25 召回候选 chunk 数，最终会与向量召回做融合，默认50
HYBRID_RRF_K = 60                       # RRF 融合平滑系数，值越大越强调多路召回共同命中，默认60
HYBRID_CANDIDATE_LIMIT = 5             # RRF 融合后送入本地重排器的最大候选 chunk 数，默认50
RECALL_K = 20                           # 本地重排后保留并送入问答的 chunk 数，默认20
...
MAX_EXTERNAL_QUERY_LIMIT = 5            # 单个外部数据源单次请求查询返回限制，防止过大被限流，默认10
DEFAULT_EXTERNAL_FINAL_LIMIT = 10       # 所有外部数据源检索返回总数限制，默认50
MAX_SEARCH_NUM = 10                     # 本地+外部检索总返回数限制，默认50
